...
IF EXIST "Schoolmate.Database.Conn.dll" (
    ECHO. && ECHO # 当前目录存在解密程序 .\Schoolmate.Database.Conn.dll && ECHO. || GOTO error
) ELSE (
    ECHO. && ECHO # 当前目录下未发现解密程序 Schoolmate.Database.Conn.dll && GOTO end
)
...
COPY "Schoolmate.Database.Conn.dll" %TEMP%\%T%\"Schoolmate.Database.Conn.exe" /V /Y || GOTO error
ECHO.
